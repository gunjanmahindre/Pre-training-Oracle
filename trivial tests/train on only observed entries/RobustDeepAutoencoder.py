...
class RDAE(object):
    ...
    """
    def __init__(self, sess, layers_sizes, lambda_=1.0, error = 1.0e-7, is_tuning=0):
        """
        ...
        self.error = error
        self.errors=[]
        self.AE = DAE.Deep_Autoencoder( sess = sess, input_dim_list = self.layers_sizes, is_tuning = is_tuning)

    def fit(self, X, Y, sess, learning_rate=0.15, inner_iteration = 50,
            iteration=20, batch_size=50, verbose=False, is_tuning=0):
...
# if __name__ == "__main__":
    ...
    with tf.compat.v1.Session() as sess:
        ...
        # create object rae
        rae = RDAE(sess = sess, lambda_= 500000, layers_sizes=[R,hidden_layer_size], is_tuning = is_tuning)
        ...
        # L, S, cost = rae.fit(data4, data4_ori, sess = sess, learning_rate=learning_rate_alpha, batch_size =batch_size,inner_iteration =inner_iteration,iteration=1, verbose=True, is_tuning=0)
        ...
        # L, S, cost = rae.fit(data5, data5_ori, sess = sess, learning_rate=learning_rate_alpha, batch_size =batch_size,inner_iteration =inner_iteration,iteration=1, verbose=True, is_tuning=0)
        ...
        # L, S, cost = rae.fit(data6, data6_ori, sess = sess, learning_rate=learning_rate_alpha, batch_size =batch_size,inner_iteration =inner_iteration,iteration=1, verbose=True, is_tuning=0)
        # print ("after training on nw3: cost = ", cost)

        print ("Training only on observed entries of the target network------------------------------------------------------")

        # Process data7 - FINE TUNING---------------------------------------------------------------
        ...
        L, S, cost = rae.fit(data_test, data_test_original, sess = sess, learning_rate=learning_rate_alpha, batch_size =batch_size,inner_iteration =inner_iteration,iteration=1, verbose=True, is_tuning=is_tuning)
        print ("after training on nw3: cost = ", cost)

        print ("Trained only on observed entries of the target network------------------------------------------------------")
        ...
        #     R.to_csv('/content/drive/MyDrive/PhD work/Projects/parameter estimation/virgili more results/fine tuning/' + ss + '/R_' + str(fraction) + '.csv', index = False)
