...
# if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        #     R.to_csv('/content/drive/MyDrive/PhD work/Projects/parameter estimation/virgili more results/' + ss + '/R_' + str(fraction) + '.csv', index = False)


        # replace all the observed entries with known values
        R = pd.DataFrame(R)
        data_test = pd.DataFrame(data_test)
...
##################################################################
        print ("-------------- Calculating error only for observed entries--------------------")
        ...
        for i in range(r):
          for j in range(c):
            if hadamard_test[i,j] == 1:   # considers error on only observed entries
